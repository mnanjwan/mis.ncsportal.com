...
use App\Mail\APERFormSubmittedMail;
...
class SendAPERFormSubmittedMailJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            if (!$this->form->officer->user || !$this->form->officer->user->email) {
                Log::warning('Cannot send APER form submitted email: officer has no email', [
            ...
            Mail::to($this->form->officer->user->email)->send(
                new APERFormSubmittedMail($this->form)
            );

            Log::info('APER form submitted email sent', [
                'form_id' => $this->form->id,
                'email' => $this->form->officer->user->email,
            ]);
        } catch (\Exception $e) {
            Log::error('Failed to send APER form submitted email', [
                'form_id' => $this->form->id,
                'error' => $e->getMessage(),
            ]);
    ...
    {
        Log::error('APER form submitted email job failed after all retries', [
            'form_id' => $this->form->id,
