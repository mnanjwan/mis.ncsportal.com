...
class EmolumentTimelineController extends Controller
{
    ...
    {
        $timelines = EmolumentTimeline::orderBy('created_at', 'desc')->paginate(20);
        
        return view('dashboards.hrd.emolument-timeline', compact('timelines'));
    }

    public function create()
    {
        return view('forms.emolument-timeline.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        $existingTimeline = EmolumentTimeline::where('year', $validated['year'])->first();
        if ($existingTimeline) {
        ...
        if ($request->has('is_active') && $request->is_active) {
            EmolumentTimeline::where('is_active', true)->update(['is_active' => false]);
        }

        $validated['created_by'] = auth()->id();
        
        try {
            $timeline = EmolumentTimeline::create($validated);
            return redirect()->route('hrd.emolument-timeline')
                ->with('success', 'Emolument timeline created successfully!');
        } catch (\Exception $e) {
            return redirect()->back()
                ->withInput()
    ...
    public function extend($id)
    {
        ...
        return view('forms.emolument-timeline.extend', compact('timeline'));
    }
    ...
    public function extendStore(Request $request, $id)
    {
        ...
        return redirect()->route('hrd.emolument-timeline')
            ->with('success', 'Emolument timeline extended successfully!');
    }
}
