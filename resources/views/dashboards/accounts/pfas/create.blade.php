...
@section('title', 'Add PFA')
@section('page-title', 'Add PFA')
...
@section('content')
    ...
    <div class="kt-card">
        <div class="kt-card-header">
            <h3 class="kt-card-title">Create PFA</h3>
        </div>
        <div class="kt-card-content">
            <form method="POST" action="{{ route('accounts.pfas.store') }}" class="flex flex-col gap-5 max-w-xl">
                @csrf
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div class="flex flex-col gap-1">
                        ...
                        <input type="text" name="rsa_prefix" class="kt-input" value="{{ old('rsa_prefix', 'PEN') }}" required maxlength="20" />
                        <small class="text-muted">Example: PEN</small>
                    </div>
                    <div class="flex flex-col gap-1">
                        <label class="kt-form-label">Digits After Prefix <span class="text-danger">*</span></label>
                        <input type="number" name="rsa_digits" class="kt-input" value="{{ old('rsa_digits', 12) }}" required min="1" max="50" />
                        <small class="text-muted">Example: 12 means RSA is prefix + 12 digits.</small>
                    </div>
                </div>
