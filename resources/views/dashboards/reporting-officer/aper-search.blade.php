    ...
    <div class="grid gap-5 lg:gap-7.5">
            ...
            <div class="kt-card bg-info/10 border border-info/20">
                <div class="kt-card-content p-4">
                    <div class="flex items-center gap-3">
                        ...
                        <p class="text-sm font-medium text-info">Your Role: <strong>{{ $rosterRole }}</strong> - You can create
                            APER forms for officers in your command.</p>
                    </div>
                </div>
            </div>
        @endif
        ...
        <div class="kt-card">
            ...
            <div class="kt-card-content">
                <form method="GET"
                    action="{{ auth()->user()->hasRole('Staff Officer') || auth()->user()->hasRole('HRD') ? route('staff-officer.aper-forms.reporting-officer.search') : route('officer.aper-forms.search-officers') }}"
                    class="flex gap-3">
                        ...
                        <table class="kt-table" style="min-width: 800px; width: 100%;">
                            <thead>
                                <tr class="border-b border-border">
                                    <th class="text-left py-3 px-4 font-semibold text-sm text-secondary-foreground">Service
                                        Number</th>
                                    ...
                                    <th class="text-left py-3 px-4 font-semibold text-sm text-secondary-foreground">Roster Role
                                    </th>
                            ...
                            <tbody>
                                @foreach($officers as $officer)
                                    <tr class="border-b border-border last:border-0 hover:bg-muted/50 transition-colors">
                                        <td class="py-3 px-4">
                                            <span
                                                class="text-sm font-medium text-foreground">{{ $officer->service_number ?? 'N/A' }}</span>
                                        </td>
                                        <td class="py-3 px-4">
                                            <span class="text-sm text-foreground">{{ $officer->initials }}
                                                {{ $officer->surname }}</span>
                                        </td>
                                        <td class="py-3 px-4">
                                            <span
                                        ...
                                        <td class="py-3 px-4 text-right">
                                            @if($officer->can_countersign)
                                                <a href="{{ route('officer.aper-forms.countersigning', $officer->pending_form->id) }}"
                                                    class="kt-btn kt-btn-sm kt-btn-success">
                                                    <i class="ki-filled ki-pen"></i> Counter Sign
                                                </a>
                                            @elseif($officer->is_subordinate || auth()->user()->hasAnyRole(['Staff Officer', 'HRD']))
                                            ...
                                            @else
                                                <span class="text-xs text-muted-foreground italic">No Action Available</span>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
            ...
            <div class="kt-card">
                <div class="kt-card-content">
                    <div class="text-center py-12">
                        ...
                        <p class="text-secondary-foreground">Use the search above to find officers and access their APER forms.
                        </p>
                    </div>
                </div>
            </div>
