...
<div class="grid gap-5 lg:gap-7.5">
    ...
    <!-- Approved Manning Requests Section -->
        ...
        <div class="kt-card">
            ...
            <div class="kt-card-content">
                <p class="text-sm text-secondary-foreground mb-4">
                    Your approved manning requests. Officers will appear here once HRD has matched them.
                </p>
                <div class="overflow-x-auto">
                    @foreach($approvedManningRequestsWithMatches as $request)
                        @php
                            $matchedItems = $request->items->where('matched_officer_id', '!=', null);
                            $unmatchedItems = $request->items->where('matched_officer_id', '==', null);
                            $hasMatchedOfficers = $matchedItems->count() > 0;
                        @endphp
                            ...
                            <div class="flex items-center justify-between mb-4">
                                <div>
                                    ...
                                    <p class="text-xs text-secondary-foreground mt-1">
                                        ...
                                        @if($hasMatchedOfficers)
                                            <span class="kt-badge kt-badge-success kt-badge-xs ml-2">Officers Matched</span>
                                        @else
                                            <span class="kt-badge kt-badge-warning kt-badge-xs ml-2">Waiting for HRD Matching</span>
                                        @endif
                                    </p>
                                </div>
                            ...
                            @else
                                <div class="bg-muted/50 rounded-lg p-4 text-center">
                                    <p class="text-sm text-secondary-foreground">
                                        ...
                                        This request is approved and waiting for HRD to match officers. You will be notified when officers are matched.
                                    </p>
                                    <div class="mt-3 text-xs text-secondary-foreground">
                                        <p>Requested positions:</p>
                                        <ul class="list-disc list-inside mt-1">
                                            @foreach($request->items as $item)
                                                <li>{{ $item->rank }} (Qty: {{ $item->quantity_needed }})</li>
                                            @endforeach
                                        </ul>
                                    </div>
                                </div>
