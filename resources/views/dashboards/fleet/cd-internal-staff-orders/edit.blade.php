...
@section('title', 'Edit Internal Staff Order (Transport)')
@section('page-title', 'Edit Internal Staff Order (Transport)')
...
@section('content')
    <div class="grid gap-5 lg:gap-7.5">
        ...
        <div class="flex items-center justify-between">
            <a href="{{ route('fleet.cd.internal-staff-orders.show', $order->id) }}" class="kt-btn kt-btn-sm kt-btn-ghost">
        ...
        <div class="kt-card">
            <div class="kt-card-header">
                ...
                <div class="kt-card-toolbar">
                    <span class="kt-badge kt-badge-info">DRAFT - Editable</span>
                </div>
            </div>
            <div class="kt-card-content">
                <form action="{{ route('fleet.cd.internal-staff-orders.update', $order->id) }}" method="POST" id="internalStaffOrderForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="flex flex-col gap-5">
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <span class="text-xs text-secondary-foreground">Order number must be unique.</span>
                            @error('order_number')
                            ...
                            <div class="relative">
                                ...
                                <button type="button" 
                                    ...
                                    <span id="officer_select_text">
                                        @if(old('officer_id', $order->officer_id))
                                            @php $selectedOfficer = $officers->find(old('officer_id', $order->officer_id)); @endphp
                        ...
                        <div id="current-assignment-section" class="{{ $order->officer_id ? '' : 'hidden' }}">
                                ...
                                <div class="kt-card-content p-4">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        <div class="flex flex-col gap-1">
                                            ...
                                            <p class="text-sm font-medium" id="current-command">{{ $command->name ?? 'N/A' }}</p>
                                        </div>
                                        <div class="flex flex-col gap-1">
                                            <label class="text-xs text-secondary-foreground">Current Unit</label>
                                            <p class="text-sm font-medium" id="current-unit">{{ $order->current_unit ?? 'Not Assigned' }}</p>
                                        </div>
                                        <div class="flex flex-col gap-1">
                                            <label class="text-xs text-secondary-foreground">Current Role</label>
                                            <p class="text-sm font-medium" id="current-role">{{ $order->current_role ?? 'Not Assigned' }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <div class="relative">
                                <input type="hidden" name="target_unit" id="target_unit" value="{{ old('target_unit', $order->target_unit) ?? '' }}" required>
                                <button type="button" 
                                    ...
                                    <span id="target_unit_select_text">{{ old('target_unit', $order->target_unit) ? old('target_unit', $order->target_unit) : 'Select target unit...' }}</span>
                                    <i class="ki-filled ki-down text-gray-400"></i>
                                </button>
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <div class="relative">
                                <input type="hidden" name="target_role" id="target_role" value="{{ old('target_role', $order->target_role) ?? '' }}" required>
                                <button type="button" 
                                    ...
                                    <span id="target_role_select_text">{{ old('target_role', $order->target_role) ? (old('target_role', $order->target_role) === 'Member' ? 'Member' : (old('target_role', $order->target_role) === '2IC' ? '2IC (Second In Command)' : 'OIC (Officer In Charge)')) : 'Select target role...' }}</span>
                                    <i class="ki-filled ki-down text-gray-400"></i>
                                </button>
                        ...
                        <div class="flex items-center gap-3 pt-4 border-t border-border">
                            ...
                            <a href="{{ route('fleet.cd.internal-staff-orders.show', $order->id) }}" class="kt-btn kt-btn-ghost">
                                Cancel
                            </a>
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Set initial selected officer
            @if(old('officer_id', $order->officer_id))
                const selectedOfficer = officers.find(o => o.id == {{ old('officer_id', $order->officer_id) }});
                if (selectedOfficer) {
            ...
            fetch('{{ route("fleet.cd.internal-staff-orders.get-officer-assignment") }}', {
                method: 'POST',
                headers: {
        ...
        function checkConflicts() {
            ...
            fetch('{{ route("fleet.cd.internal-staff-orders.check-conflicts") }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        // Load current assignment on page load if officer is already selected
        @if($order->officer_id)
            officerHiddenInput.dispatchEvent(new Event('change'));
        @endif
        });
