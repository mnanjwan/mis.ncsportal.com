...
@section('content')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const form = document.querySelector('form[action="{{ route('hrd.role-assignments.store') }}"]');
            ...
            async function handleFormSubmit(e) {
                    ...
                    try {
                        ...
                        if (data.success) {
                            if (data.redirect) {
                                ...
                                window.location.href = '{{ route('hrd.role-assignments') }}';
                            }
                        } else {
                ...
                try {
                    const response = await fetch(`{{ route('hrd.role-assignments.check-existing-roles') }}?officer_id=${officerId}`, {
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest',
                            'Accept': 'application/json'
                        }
