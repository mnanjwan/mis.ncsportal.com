            ...
            <div class="kt-card-content">
                                    ...
                                    <th class="text-left py-3 px-4 font-semibold text-sm text-secondary-foreground">
                                        <a href="{{ request()->fullUrlWithQuery(['sort_by' => 'year', 'sort_order' => request('sort_by') === 'year' && request('sort_order') === 'asc' ? 'desc' : 'asc']) }}"
                                            ...
                                            @if(request('sort_by') === 'year')
                                                <i
                                                    class="ki-filled ki-arrow-{{ request('sort_order') === 'asc' ? 'up' : 'down' }} text-xs"></i>
                                            @else
                                                                ...
                                                                <div class="flex items-center justify-end gap-2">
                                                                    ...
                                                                    @if(($list->eligible_officers_count ?? 0) > 0)
                                                                        @if(($list->status ?? 'DRAFT') === 'DRAFT')
                                                                            <form action="{{ route('hrd.promotion-eligibility.finalize', $list->id) }}"
                                                                        ...
                                                                        @elseif(($list->status ?? '') === 'FINALIZED')
                                                                            <form
                                                                                action="{{ route('hrd.promotion-eligibility.submit-to-board', $list->id) }}"
                                                                                method="POST" class="inline">
                                                                                @csrf
                                                                    ...
                                                                    @endif
                                                                    @if(($list->eligible_officers_count ?? 0) == 0)
                                                                        <button type="button" data-kt-modal-toggle="#delete-modal-{{ $list->id }}"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="py-12 text-center">
                                            ...
                                            <a href="{{ route('hrd.promotion-eligibility.create') }}"
                ...
                <div class="lg:hidden">
                    <div class="flex flex-col gap-4">
                        @forelse($lists as $list)
                            <div
                                class="flex items-center justify-between p-4 rounded-lg bg-muted/50 border border-input hover:bg-muted transition-colors">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    @if(($list->eligible_officers_count ?? 0) > 0)
                                        @if(($list->status ?? 'DRAFT') === 'DRAFT')
                                            <form action="{{ route('hrd.promotion-eligibility.finalize', $list->id) }}" method="POST"
                                                class="inline">
                                                @csrf
                                                <button type="submit" class="kt-btn kt-btn-sm kt-btn-secondary">
                                                    Finalize
                                        ...
                                        @elseif(($list->status ?? '') === 'FINALIZED')
                                            <form action="{{ route('hrd.promotion-eligibility.submit-to-board', $list->id) }}"
                                                method="POST" class="inline">
                                                @csrf
                                                <button type="submit" class="kt-btn kt-btn-sm kt-btn-primary">
                                                    Submit
                ...
                <div class="kt-modal-content max-w-[400px]">
                    ...
                    <div class="kt-modal-body py-5 px-5">
                        <p class="text-sm text-secondary-foreground">
                            Are you sure you want to delete the promotion eligibility list for year
