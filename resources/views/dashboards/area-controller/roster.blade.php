...
@section('breadcrumbs')
    <a class="text-secondary-foreground hover:text-primary" href="{{ route('area-controller.dashboard') }}">Area
        Controller</a>
    <span>/</span>
                                ...
                                @foreach($rosters as $roster)
                                    <tr class="border-b border-border last:border-0 hover:bg-muted/50 transition-colors">
                                        ...
                                        <td class="py-3 px-4 text-sm text-secondary-foreground">
                                            {{ $roster->command->name ?? 'N/A' }}</td>
                                        <td class="py-3 px-4 text-sm text-secondary-foreground">
                                            {{ $roster->roster_period_start->format('M d') }} -
                                            {{ $roster->roster_period_end->format('M d, Y') }}
                                        </td>
                                        <td class="py-3 px-4 text-sm text-secondary-foreground">
                                            {{ $roster->preparedBy->email ?? 'N/A' }}</td>
                                        <td class="py-3 px-4 text-sm text-secondary-foreground">
                                        ...
                                        <td class="py-3 px-4 text-sm text-secondary-foreground">{{ $roster->assignments->count() }}
                                            assignments</td>
                                        <td class="py-3 px-4 text-right">
                                            <a href="{{ route('area-controller.roster.show', $roster->id) }}"
