...
document.addEventListener('DOMContentLoaded', function() {
    @php
        // We prepare the array purely in PHP first
        $mapped = [];
        foreach ($drugs as $d) {
            $qty = 0;
            if (isset($centralStock[$d->id])) {
                $qty = $centralStock[$d->id]->sum('quantity');
            }
    ...
    // Inject the PHP array into JS safely
    const drugsData = @json($mapped);
    ...
    function addItem() {
        ...
        // Update name attributes and IDs with current index
