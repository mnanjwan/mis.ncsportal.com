...
@section('title', 'Validate Emolument')
@section('page-title', 'Validate Emolument')

@section('breadcrumbs')
    <a class="text-secondary-foreground hover:text-primary" href="{{ route('validator.dashboard') }}">Validator</a>
    <span>/</span>
    <span class="text-primary">Validate Emolument</span>
@endsection
...
@section('content')
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-5 lg:gap-9 mb-5 lg:mb-10">
        <div class="xl:col-span-2 space-y-5">
            ...
            <!-- Validation Form -->
            <form class="kt-card" action="{{ route('emolument.process-validation', $emolument->id) }}" method="POST">
                @csrf
                ...
                <div class="kt-card-header">
                    <h3 class="kt-card-title">Validation Decision</h3>
                </div>
                <div class="kt-card-content space-y-5">
                    <div class="flex flex-col gap-1">
                        ...
                        <div class="relative">
                            <input type="hidden" name="validation_status" id="validation_status" value="{{ old('validation_status') ?? '' }}" required>
                            <button type="button" 
                                    id="validation_status_select_trigger" 
                                    class="kt-input w-full text-left flex items-center justify-between cursor-pointer @error('validation_status') border-red-500 @enderror">
                                <span id="validation_status_select_text">{{ old('validation_status') ? (old('validation_status') === 'APPROVED' ? 'Approve' : 'Reject') : 'Select Decision' }}</span>
                                <i class="ki-filled ki-down text-gray-400"></i>
                            </button>
                            <div id="validation_status_dropdown" 
                                 class="absolute z-50 w-full mt-1 bg-white border border-input rounded-lg shadow-lg hidden">
                                <div class="p-3 border-b border-input">
                                    <input type="text" 
                                           id="validation_status_search_input" 
                                           class="kt-input w-full pl-10" 
                                           placeholder="Search..."
                                           autocomplete="off">
                                </div>
                                <div id="validation_status_options" class="max-h-60 overflow-y-auto"></div>
                            </div>
                        </div>
        ...
        <div class="xl:col-span-1">
            ...
            <div class="kt-card bg-accent/50">
                <div class="kt-card-header">
                    <h3 class="kt-card-title">Validation Guidelines</h3>
                </div>
                <div class="kt-card-content">
                    <div class="flex flex-col gap-3 text-sm">
                        <p class="text-xs text-secondary-foreground">
                            Please perform final verification before approving for payment processing.
                        </p>
                        <ul class="text-xs text-secondary-foreground list-disc list-inside space-y-1">
                            <li>Verify all information matches records</li>
                            <li>Check assessment comments</li>
    ...
    <script>
        ...
        // Initialize validation status select
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (document.getElementById('validation_status_select_trigger')) {
                createSearchableSelect({
                    triggerId: 'validation_status_select_trigger',
                    hiddenInputId: 'validation_status',
                    dropdownId: 'validation_status_dropdown',
                    searchInputId: 'validation_status_search_input',
                    optionsContainerId: 'validation_status_options',
                    displayTextId: 'validation_status_select_text',
