...
@section('title', 'Edit Manning Level Request')
@section('page-title', 'Edit Manning Level Request')
...
@section('breadcrumbs')
    ...
    <span>/</span>
    <a class="text-secondary-foreground hover:text-primary" href="{{ route('staff-officer.manning-level.show', $request->id) }}">View</a>
    <span>/</span>
...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-5 lg:gap-9 mb-5 lg:mb-10">
    <div class="xl:col-span-2 space-y-5">
            ...
            <div class="kt-card-content p-5">
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex flex-col gap-1">
                        <span class="text-sm font-semibold text-mono">Edit Manning Level Request</span>
                        <span class="text-xs text-secondary-foreground">
                            Update your manning level request. Only DRAFT requests can be edited.
                        </span>
                    </div>
                </div>
            </div>
        ...
        <!-- Form -->
        <form class="kt-card" id="edit-manning-request-form" method="POST" action="{{ route('staff-officer.manning-level.update', $request->id) }}">
            @csrf
            @method('PUT')
            ...
            <div class="kt-card-content space-y-5">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <textarea class="kt-input" placeholder="Additional notes or remarks" name="notes" rows="4">{{ old('notes', $request->notes) }}</textarea>
                </div>
            </div>
            <div class="kt-card-footer flex justify-end items-center flex-wrap gap-3">
                <a class="kt-btn kt-btn-outline" href="{{ route('staff-officer.manning-level.show', $request->id) }}">Cancel</a>
                    ...
                    <div class="kt-card shadow-none bg-info/10 border border-info/20">
                        <div class="kt-card-content p-4">
                            <p class="text-xs text-secondary-foreground mb-2">
                                <strong class="text-mono">Update Process:</strong>
                            </p>
                            <ul class="text-xs text-secondary-foreground mt-2 list-disc list-inside space-y-1">
                                <li>Modify existing items or add new ones</li>
                                <li>Remove items that are no longer needed</li>
                                <li>Update notes as necessary</li>
                                <li>Save changes to update the request</li>
                            </ul>
                        </div>
                    </div>
                    <p class="text-xs text-secondary-foreground">
                        Only DRAFT requests can be edited. Once submitted, the request cannot be modified.
                    </p>
                </div>
            </div>
        </div>
...
function createItemTemplate(index, itemData = null) {
    ...
    const qualsHtml = qualifications.map(qual => {
        const selected = itemData && itemData.qualification_requirement === qual ? 'selected' : '';
        return `<option value="${qual}" ${selected}>${qual}</option>`;
    }).join('');
    
    const quantity = itemData ? itemData.quantity_needed : '';
    const sexReq = itemData ? itemData.sex_requirement : 'ANY';
    const qualReq = itemData ? itemData.qualification_requirement : '';
    const showCustomQual = itemData && qualReq && !qualifications.includes(qualReq);
            ...
            <div class="kt-card-content p-4 space-y-4">
                ...
                <div class="grid sm:grid-cols-2 gap-4">
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <input class="kt-input" type="number" name="items[${index}][quantity_needed]" min="1" placeholder="Number of officers" value="${quantity}" required/>
                    </div>
                </div>
                <div class="grid sm:grid-cols-2 gap-4">
                    <div class="flex flex-col gap-1">
                        ...
                        <select class="kt-input" name="items[${index}][sex_requirement]">
                            <option value="ANY" ${sexReq === 'ANY' ? 'selected' : ''}>Any</option>
                            <option value="M" ${sexReq === 'M' ? 'selected' : ''}>Male</option>
                            <option value="F" ${sexReq === 'F' ? 'selected' : ''}>Female</option>
                        </select>
                    </div>
                    <div class="flex flex-col gap-1">
                        <label class="kt-form-label font-normal text-mono text-xs">Qualification Requirement</label>
                        <select class="kt-input" name="items[${index}][qualification_requirement]" id="qual-select-${index}" ${showCustomQual ? 'style="display: none;"' : ''}>
                            <option value="">Any Qualification</option>
                            ${qualsHtml}
                        </select>
                        <input type="text" class="kt-input mt-1" name="items[${index}][qualification_custom]" placeholder="Enter custom qualification" value="${showCustomQual ? qualReq : ''}" style="display: ${showCustomQual ? 'block' : 'none'};" id="qual-custom-${index}"/>
                        <button type="button" class="kt-btn kt-btn-sm kt-btn-ghost mt-1" onclick="toggleCustomQual(${index})" id="qual-toggle-${index}">
                            <i class="ki-filled ki-${showCustomQual ? 'cross' : 'plus'}"></i> ${showCustomQual ? 'Use List' : 'Custom'}
                        </button>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (existingItems.length > 0) {
        ...
        // Add first item if no existing items
        addItem();
    }
    ...
    document.getElementById('edit-manning-request-form').addEventListener('submit', function(e) {
