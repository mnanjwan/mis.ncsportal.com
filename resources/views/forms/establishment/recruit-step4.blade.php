...
@section('title', 'Add New Recruit - Step 4: Next of Kin')
@section('page-title', 'Add New Recruit - Step 4: Next of Kin')
...
@section('breadcrumbs')
    ...
    <span>/</span>
    <span class="text-primary">Step 4: Next of Kin</span>
@endsection
...
<div class="grid gap-5 lg:gap-7.5">
    ...
    <div class="kt-card">
        <div class="kt-card-content p-4 lg:p-5">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-2">
                ...
                <div class="flex items-center gap-2">
                    <div class="size-8 rounded-full flex items-center justify-center font-semibold text-sm" style="background-color: #068b57; color: white;">4</div>
                    <span class="text-xs sm:text-sm font-medium" style="color: #068b57;">Next of Kin</span>
                </div>
                <div class="flex items-center gap-2">
    ...
    <div class="kt-card">
        ...
        <div class="kt-card-content">
            ...
            <form id="recruit-step4-form" method="POST" action="{{ route('establishment.new-recruits.step4.save') }}" enctype="multipart/form-data" class="flex flex-col gap-5 w-full overflow-hidden">
                @csrf
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 pt-5 border-t border-input">
                    <button type="button" onclick="window.location.href='{{ route('establishment.new-recruits.step3') }}'" class="kt-btn kt-btn-secondary w-full sm:flex-1 whitespace-nowrap">Previous</button>
                    <button type="submit" class="kt-btn text-white w-full sm:flex-1 whitespace-nowrap" id="submit-btn" style="background-color: #068b57; border-color: #068b57;">Next: Preview</button>
                </div>
                    </div>
...
document.getElementById('recruit-step4-form').addEventListener('submit', function(e) {
    e.preventDefault();
    ...
    submitBtn.innerHTML = '⏳ Submitting...';
            ...
            // Submit the form
            document.getElementById('recruit-step4-form').submit();
        }
    });
});

// Clear errors on input - use event delegation for dynamically added fields
document.getElementById('recruit-step4-form').addEventListener('input', function(e) {
...
document.getElementById('recruit-step4-form').addEventListener('change', function(e) {
    if (e.target.matches('.nok-name, .nok-relationship, .nok-phone, .nok-email, .nok-address')) {
        const errorSpan = e.target.parentElement?.querySelector('.error-message');
        if (errorSpan) {
            errorSpan.textContent = '';
